...
OS에 따라 다음 폴더에 `.sh3p` 파일을 복사 (폴더 없으면 만들기):

| OS | 플러그인 폴더 경로 |
|---|---|
| **Windows** | `%APPDATA%\eTeks\SweetHome3D\plugins\` (탐색기 주소창에 그대로 붙여넣기) |
| **macOS** | `~/Library/Application Support/eTeks/SweetHome3D/plugins/` |
| **Linux** | `~/.eteks/sweethome3d/plugins/` |

> 💡 **Windows 더블클릭 방법**도 있지만 ([공식 안내](https://github.com/adizanni/ExportToHASS#plugin-installation)) 일부 사용자에게 설치가 안 되는 보고가 있어요. 위 폴더에 직접 복사하는 게 가장 확실.

### 1-3. Sweet Home 3D 재시작

완전 종료 후 다시 실행.

### 1-4. 설치 확인

메뉴 **`도구(Tools) → Export obj to HASS`** 항목이 보이면 OK ✅

> 한글판 메뉴 이름이 다를 수 있어요. `Tools` 또는 `도구` 메뉴를 펼쳐서 `Export obj to HASS` 항목 찾기.
...
도면 안에서 각 조명을 클릭 → Properties → Name 에 이름 정할 때 **다음 글자만** 사용:
- 영문 대소문자 (`a-z`, `A-Z`)
- 숫자 (`0-9`)
- 언더스코어 (`_`)

**공백·하이픈·한글·특수문자 모두 안 됨.** 사용 시 ExportToHASS가 export 시 거부합니다.

| ❌ 안 됨 | ✅ 됨 |
|---|---|
| `안방 조명` | `Bedroom_Master` 또는 `BedroomMaster` |
| `Living Main Light` (공백) | `Living_Main_Light` 또는 `LivingMainLight` |
| `Kitchen-Dining` (하이픈) | `Kitchen_Dining` |
| `거실 메인` | `Living_Main` |

> 💡 Home Assistant 안의 entity ID는 한글이어도 OK. SH3D 모델 안의 **객체 이름**만 영문/숫자/언더스코어로 제한됩니다.
...
Sweet Home 3D에서 도면 열어 둔 상태에서:

**메뉴 `도구(Tools) → Export obj to HASS` 클릭** → zip 파일로 저장할 위치/이름 선택 (예: 바탕화면에 `myhome_export.zip`)

> ExportToHASS는 폴더가 아니라 **zip 파일 하나**로 출력합니다.

### 3-2. zip 압축 풀기

저장된 `myhome_export.zip` 파일을 압축 풀기:
- **Windows**: zip 파일 우클릭 → "압축 풀기" → 같은 위치에 폴더 생성
- **macOS**: zip 파일 더블 클릭 → 자동 압축 해제

압축 풀린 폴더 안에 다음 파일들이 있어야 OK:

```
myhome_export/
├── home.obj           ← 3D 모델 데이터
├── home.mtl           ← 색상·재질 정보
├── home_<...>.jpeg    ← 텍스처 이미지 14개 정도
└── home.json          ← 부가 정보 (object_id 목록)
```

이 폴더가 다음 단계에서 사용할 작업 폴더입니다.
...
### object_id 알아내는 두 가지 방법

**방법 A — `home.nodes.txt` 파일 보기** (가장 빠름)

5단계에서 자동 생성된 `dist/home.nodes.txt` 열어서 mesh node 이름 복사.

**방법 B — 카드에서 직접 더블클릭** (floor3d-card 자체 기능)

8단계 카드를 entities 비워서 먼저 띄운 다음, 3D 화면의 해당 객체 (예: 거실 천장 조명)를 **더블클릭** → object_id 표시. 이게 시각적으로 가장 직관적.

```yaml
# entities 비워서 모델만 띄우기
type: custom:floor3d-card
path: /local/floor3d/
objfile: home.glb
shadow: 'yes'
...
저장 후 화면에서 클릭하고 싶은 조명·가구를 더블클릭하면 popup으로 object_id 확인 가능.

### 라이트 위치를 시각적으로 확인하고 싶다면

조명 픽스처가 투명 처리되어 위치 파악이 어렵다면 5단계를 옵션 켜서 다시:
...
### Sweet Home 3D 메뉴에 `Export obj to HASS` 항목이 없어요

메뉴는 **`도구(Tools)`** 안에 있습니다 (`파일/File` 메뉴 아님). 그래도 안 보이면 플러그인 설치 미완료:

플러그인 파일 위치 확인:
- **Windows**: `%APPDATA%\eTeks\SweetHome3D\plugins\` 에 `ExportToHASSPlugin.sh3p`
- **macOS**: `~/Library/Application Support/eTeks/SweetHome3D/plugins/`
- **Linux**: `~/.eteks/sweethome3d/plugins/`

확인 후 Sweet Home 3D를 **완전히 종료 후** 다시 실행.
