...
|---|---|---|
| 플러그인 | `adizanni/ExportToHASS` | `shmuelzon/home-assistant-floor-plan` |
...
| | |
|---|---|
| Sweet Home 3D | 무료 GPL OSS — https://www.sweethome3d.com/download.jsp<br/>한국어 가이드 (글): [ray 님 블로그](https://naver.me/xcg9imY8)<br/>한국어 가이드 (영상): [YouTube 튜토리얼](https://youtu.be/6JMdpXlp1po?si=4PCwZQzDjKKhFDU9) |
| ExportToHASS 플러그인 (`.sh3p`) | https://github.com/adizanni/ExportToHASS/releases/latest/download/ExportToHASSPlugin.sh3p |
| Python | 3.11+ |
| Home Assistant | + [HACS](https://www.hacs.xyz/) (`floor3d-card` 설치용) |

### ExportToHASS 플러그인 설치 (1회)

1. 위 URL에서 `ExportToHASSPlugin.sh3p` 다운로드
2. OS에 따라 다음 폴더에 복사 (없으면 폴더 만들기):
...
4. 메뉴 **`도구(Tools) → Export obj to HASS`** 항목이 보이면 OK ✅
...
5. 조명 이름을 알아보기 쉽게 변경 (예: `Living_Main_Light`, `Bedroom_Master`)
6. `myhome.sh3d`로 저장

### ⚠️ 객체 이름 규칙 (꼭 지킬 것)

`ExportToHASS` 플러그인은 **`a-z`, `A-Z`, `0-9`, `_` (언더스코어)** 만 허용합니다. 공백·하이픈·한글·특수문자는 모두 거부됩니다.

| ❌ 안 됨 | ✅ 됨 |
|---|---|
| `안방 조명` (한글+공백) | `Bedroom_Master` |
| `Living Main Light` (공백) | `Living_Main_Light` |
...
이 이름이 GLB 안의 mesh node 이름으로 변환되고, 카드 YAML에서 `object_id` 값으로 그대로 쓰입니다. 명확하고 규칙 지킨 이름이면 매핑이 매우 쉬워요.

---

## 2. ExportToHASS로 내보내기

1. SH3D에서 도면 열어 둔 상태에서 메뉴 **`도구(Tools) → Export obj to HASS`** 클릭
2. **zip 파일** 저장 위치/이름 선택 (예: `~/Desktop/myhome_export.zip`)
3. 저장된 zip 파일을 **압축 풀기** (Windows 우클릭 → "압축 풀기", macOS 더블 클릭)
4. 풀린 폴더 안에 다음 파일들이 있어야 OK:
   ```
   home.obj
   home.mtl
   home_<여러>.jpeg     # 텍스처 14개 정도
...
| SH3D `Tools / 도구` 메뉴에 `Export obj to HASS` 항목 안 보임 | 플러그인 설치 실패. 다음 위치에 `.sh3p` 있는지 확인 후 SH3D 재시작 — Windows: `%APPDATA%\eTeks\SweetHome3D\plugins\` / macOS: `~/Library/Application Support/eTeks/SweetHome3D/plugins/` / Linux: `~/.eteks/sweethome3d/plugins/` |
| 객체 이름에 한글·공백 썼는데 export가 거부됨 | ExportToHASS는 `a-zA-Z0-9_` 만 허용. SH3D에서 객체 이름 다 영문/숫자/언더스코어로 변경 후 재 export |
